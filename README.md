...
```
your project
├─ scripts/store-shots/       ← YOU own this — keep it all in one folder
│  ├─ content/
...
The folder name is yours; this guide standardizes on `scripts/store-shots/`. See
...
bunx store-shots init                     # scaffold scripts/store-shots/ + add npm scripts
```

`init` creates a working starter under `scripts/store-shots/` (data, template, CLI, and an
`assets/` folder) and adds `store:preview` / `store:build` to your `package.json`. It never
overwrites existing files — pass `--force` to replace them, `--no-scripts` to skip the
`package.json` edit, or `bunx store-shots init <dir>` to choose a different folder.
...
`store-shots init` scaffolds exactly this layout for you. Everything lives in **one dedicated
folder** so it never tangles with your app's source. This guide standardizes on
`scripts/store-shots/` (matching the package name), but the name is yours — pass it to init as
...
> Already on a different name (e.g. yaboyo's `scripts/store-screenshots/`)? It keeps working as-is —
...
- How you _produce_ the screenshots (simulator capture, manual export, etc.) is out of scope — the
  engine only consumes the finished images.
...
3. Drop real screenshots into content/assets/ using the names your template expects.
...
1. **Install & scaffold**: run `bun add -D -E github:tzwzx/store-shots`, then `bunx store-shots init`.
   This creates `scripts/store-shots/` with a working starter and adds the `store:*` scripts.
...
8. **Add assets** under `content/assets/`, named to match your `ctx.asset` paths.
